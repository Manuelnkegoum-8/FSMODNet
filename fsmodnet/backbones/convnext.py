...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
                 ...
                 out_indices=[0, 1, 2, 3], pretrained=None, backbone_dir=None
                 ):
        super().__init__()
        self.dims = dims
        ...
        if pretrained is not None:
            checkpoint = torch.hub.load_state_dict_from_url(url=pretrained, 
                                                            model_dir=backbone_dir, map_location="cpu", check_hash=True)
...
def build_convnext(modelname, pretrained,backbone_dir=None, **kw):
    assert modelname in ['convnext_xlarge_22k', 'convnext_small_1k', 'convnext_tiny_1k']
    model_para_dict = {
        'convnext_xlarge_22k': dict(
            depths=[3, 3, 27, 3],
            dims=[256, 512, 1024, 2048],
        ),
        'convnext_tiny_1k': dict(
            depths=[3, 3, 9, 3],
            dims=[96, 192, 384, 768],
        ),
        'convnext_small_1k': dict(
            depths=[3, 3, 27, 3],
            dims=[96, 192, 384, 768],
        ),
    }
